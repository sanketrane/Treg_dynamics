...
real[] shm_chi(real time, real[] y, real[] parms, real[] rdata,  int[] idata) {
  ...
  dydt[1] = theta_spline(t, psi) * (1- Chi_spline(t - ageAtBMT)) * eps_host + rho_0 * (2 * y[2] + y[1]) - (kloss + alpha + delta_0) * y[1];
  // Thymic ki lo tranistionals
  dydt[2] = theta_spline(t, psi) * (1- Chi_spline(t - ageAtBMT)) * (1 - eps_host) + kloss * y[1] - (rho_0 + alpha + delta_0) * y[2];
  ...
  // Donor naive Tregs
  // Thymic ki  hi tranistionals
  dydt[9] = theta_spline(t, psi) * Chi_spline(t - ageAtBMT) * donor_eps_spline(t) + rho_0 * (2 * y[10] + y[9]) - (kloss + alpha + delta_0) * y[9];
  // Thymic ki lo tranistionals
  dydt[10] = theta_spline(t, psi) * Chi_spline(t - ageAtBMT) * (1 - donor_eps_spline(t)) + kloss * y[9] - (rho_0 + alpha + delta_0) * y[10];
  // Peripheral ki hi tranistionals
