functions{
  ...
  real[] shm(real time, real[] y, real[] parms, real[] rdata, int[] idata) {
   ...
   real[,] solve_init(real[] tb_time,           // age at BMT
     ...
     real[] parms){
       ...
       y_init[2:ndim] = integrate_ode_rk45(shm, init_cond, tb_time[1], tb_time[2:ndim], params_init, {0.0}, x_i);
   ...
   real[] solve_ode(real time_point,            // time point of observation
     ...
     real[] parms){
       ...
       real y_hat[2, 4];
       real init_tb[4];                         // init conditions at the time of BMT
       real params[6];
       ...
       y_hat[1] = init_tb;
       y_hat[2] = to_array_1d(integrate_ode_rk45(shm, init_tb, ageAtBMT, rep_array(time_point, 1), params, {0.0}, {0}));
...
generated quantities{
  real y_init_pred[3, 4];
  real y_solve_pred1[num_pred, 4]; real y_solve_pred2[num_pred, 4];  real y_solve_pred3[num_pred, 4];
  ...
  for (i in 1:num_pred){
    ...
    //Peripheral Tregs
    // total counts
    y3_mean_pred1[i] = y_solve_pred1[i, 3] + y_solve_pred1[i, 4];
    y3_mean_pred2[i] = y_solve_pred2[i, 3] + y_solve_pred2[i, 4];
    y3_mean_pred3[i] = y_solve_pred3[i, 3] + y_solve_pred3[i, 4];
