...
real[] shm_simple(real time, real[] y, real[] parms, real[] rdata,  int[] idata) {
  real psi = parms[1];
  real alpha = parms[2];
  real lambda_D = parms[3];
  real Beta = parms[4];
  ...
  // Host naive Tregs
  // Thymic displaceable
  dydt[1] = theta_spline(time, psi) * (1- Chi_spline(time - ageAtBMT)) + Beta * y[2] - (alpha + lambda_D) * y[1];
  // Peripheral displaceable
  dydt[2] = alpha * y[1] - (Beta + lambda_D) * y[2];
  // Peripheral  Incumbent
  dydt[3] = alpha * y[4] - (Beta + lambda_I) * y[3];
    // Thymic  Incumbent
  dydt[4] = Beta * y[3] - (alpha + lambda_I) * y[4];

  // Donor naive Tregs
  // Thymic displaceable
  dydt[5] = theta_spline(time, psi) * Chi_spline(time - ageAtBMT) + Beta * y[6] - (alpha + lambda_D) * y[5];
  // Peripheral displaceable
  dydt[6] = alpha * y[5] - (Beta + lambda_D) * y[6];
...
real[] solve_init1(real ageAtBMT,
  ...
  real[] parms){
    ...
    if (ageAtBMT==40) {
      ...
      y_init[2] = to_array_1d(integrate_ode_rk45(shm_simple, init_cond, ta, rep_array(ageAtBMT, 1), params_init, {0.0}, {0}));
    }
    return y_init[2];
}

real[] solve_chi1(real solve_time, real ageAtBMT, real[] init_cond, real[] parms){
     real y_solve[6];
    real params[6];
    ...
    y0 = solve_init1(ageAtBMT, init_cond, parms);
    ...
    y_solve = to_array_1d(integrate_ode_rk45(shm_simple, init_tb, ageAtBMT, rep_array(solve_time, 1), params, {0.0}, {0}));

    return y_solve;
  }

 real[,] solve_ode_chi1(real[] solve_time, real[] ageAtBMT, real[] init_cond, real[] parms){
    int numdim = size(solve_time);
    real y_solve[numdim, 6];
    for (i in 1:numdim) {
      y_solve[i] = solve_chi1(solve_time[i], ageAtBMT[i], init_cond, parms);
    }
    return y_solve;
  }
...
// solving for total counts of thymic and peripheral naive Tregs at time of BMT assuming the youngest animal as the t0
// these counts form the initial conditions for other recipients N(0) = N_host(0)
real[] solve_init2(real ageAtBMT,
  real[] init_cond,                          // initial conditions at BMT in the youngest mouse
  real[] parms){
...
real[] solve_chi2(real solve_time, real ageAtBMT, real[] init_cond, real[] parms){
     real y_solve[12];
    real params[7];
    ...
    //solution for the initial conditions at the mean age of BMT for the group
    y0 = solve_init2(ageAtBMT, init_cond, parms);
 ...
 real[,] solve_ode_chi2(real[] solve_time, real[] ageAtBMT, real[] init_cond, real[] parms){
    int numdim = size(solve_time);
  ...
  vector math_reduce(vector global_params, vector local_params, real[] x_r, int[] x_i){
    ...
    // ODE solution -- predictions for the observed timecourse
    init_cond1[1] = y1_0 + y2_0; init_cond1[2] = y3_0 + y4_0; init_cond1[3] = y5_0 + y6_0;
    init_cond1[4] = y7_0 + y8_0; init_cond1[5] = 0; init_cond1[6] = 0; 
    // ODE solution -- predictions for the observed timecourse
    init_cond2[1] = y1_0; init_cond2[2] = y2_0; init_cond2[3] = y3_0; init_cond2[4] = y4_0;
    init_cond2[5] = y5_0; init_cond2[6] = y6_0; init_cond2[7] = y7_0; init_cond2[8] = y8_0;
    init_cond2[9] =  0; init_cond2[10] = 0; init_cond2[11] = 0; init_cond2[12] = 0;
    ...
    // ODE solution for chimera dataset -- x_r = data time and x_i = time at BMT
      simple_solve = solve_ode_chi1(solve_time, tb_time, init_cond1, to_array_1d(global_params));
      chi_solve = solve_ode_chi2(solve_time, tb_time, init_cond2, to_array_1d(global_params));

      for (i in 1:n){
        counts_thy[i] = simple_solve[i, 1] + simple_solve[i, 4] + simple_solve[i, 5];
        counts_per[i] = simple_solve[i, 2] + simple_solve[i, 3] + simple_solve[i, 6];
...
generated quantities{
  real y_simple_pred1[numPred, 6];
  real y_simple_pred2[numPred, 6];
  real y_simple_pred3[numPred, 6];
  real y_simple_pred4[numPred, 6];
  ...
  init_cond1[1] = exp(y1_0) + exp(y2_0); init_cond1[2] = exp(y3_0) + exp(y4_0); 
  init_cond1[3] = exp(y5_0) + exp(y6_0); init_cond1[4] = exp(y7_0) + exp(y8_0);
  init_cond1[5] = 0; init_cond1[6] = 0; 

  init_cond2[1] = exp(y1_0); init_cond2[2] = exp(y2_0); init_cond2[3] = exp(y3_0); init_cond2[4] = exp(y4_0);
  init_cond2[5] = exp(y5_0); init_cond2[6] = exp(y6_0); init_cond2[7] = exp(y7_0); init_cond2[8] = exp(y8_0);
  init_cond2[9] =  0; init_cond2[10] = 0; init_cond2[11] = 0; init_cond2[12] = 0;

  // predictions for the whole timecourse
  y_simple_pred1 = solve_ode_chi1(ts_pred1, tb_pred1, init_cond1, to_array_1d(global_params));
  y_simple_pred2 = solve_ode_chi1(ts_pred2, tb_pred2, init_cond1, to_array_1d(global_params));
  y_simple_pred3 = solve_ode_chi1(ts_pred3, tb_pred3, init_cond1, to_array_1d(global_params));
  y_simple_pred4 = solve_ode_chi1(ts_pred4, tb_pred4, init_cond1, to_array_1d(global_params));
  
  y_chi_pred1 = solve_ode_chi2(ts_pred1, tb_pred1, init_cond2, to_array_1d(global_params));
  y_chi_pred2 = solve_ode_chi2(ts_pred2, tb_pred2, init_cond2, to_array_1d(global_params));
  y_chi_pred3 = solve_ode_chi2(ts_pred3, tb_pred3, init_cond2, to_array_1d(global_params));
  y_chi_pred4 = solve_ode_chi2(ts_pred4, tb_pred4, init_cond2, to_array_1d(global_params));

  for (i in 1:numPred){
    counts_thy_mean_pred1[i] = y_simple_pred1[i, 1] + y_simple_pred1[i, 4] + y_simple_pred1[i, 5];
    counts_thy_mean_pred2[i] = y_simple_pred1[i, 1] + y_simple_pred1[i, 4] + y_simple_pred1[i, 5];
    counts_thy_mean_pred3[i] = y_simple_pred1[i, 1] + y_simple_pred1[i, 4] + y_simple_pred1[i, 5];
    counts_thy_mean_pred4[i] = y_simple_pred1[i, 1] + y_simple_pred1[i, 4] + y_simple_pred1[i, 5];
    
    counts_per_mean_pred1[i] = y_simple_pred1[i, 2] + y_simple_pred1[i, 3] + y_simple_pred1[i, 6];
    counts_per_mean_pred2[i] = y_simple_pred1[i, 2] + y_simple_pred1[i, 3] + y_simple_pred1[i, 6];
    counts_per_mean_pred3[i] = y_simple_pred1[i, 2] + y_simple_pred1[i, 3] + y_simple_pred1[i, 6];
    counts_per_mean_pred4[i] = y_simple_pred1[i, 2] + y_simple_pred1[i, 3] + y_simple_pred1[i, 6];
