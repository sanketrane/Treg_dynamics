functions{
 //spline 1
 // Timecourse of source population -- peripheral naive Treg cells
 real sp_numbers(real time) {
   real t0 = 40.0;     // mean mouse age at BMT for the first ageBMT bin
   real value;
   // spline fitted separately to the counts of peripheral naive Treg cells to estimate the parameters
   real b0  = 6.2; real nu = 0.0027 ;
   //best fitting spline
   value = 10^b0 * exp(-nu * (time-t0));
   return value;
 }

 // Total influx into the memory Treg cell compartment from peripheral naive Treg cells (cells/day)
 real theta_spline(real time, real psi){
  ...
  real Chi_spline(real time) {
    ...
    real value;  real chiEst = 0.77;   real qEst = 0.0155;
...
// spline3 --
// proportions of ki67hi cells in the donor-derived -- peripheral naive Treg cells 
real donor_eps_spline(real time_post_BMT){
   ...
   real b0 = 0.085; real eps_f = 0.001;
   return b0 * exp(eps_f * time_post_BMT);
}
...
real[] shm_chi(real time, real[] y, real[] parms, real[] rdata,  int[] idata) {
  ...
  real eps_host = 0.125;     // Mean Ki67 hi fraction in host-BM-derived FoxP3 positive Sp4 T cells
 ...
 real[,] solve_ode_chi(real[] solve_time, real[] ageAtBMT, real[] init_cond, real[] parms){
  ...
  vector math_reduce(vector global_params, vector local_params, real[] x_r, int[] x_i){
    ...
    // PDE solution for chimera dataset -- x_r = data time and x_i = time at BMT
      ...
      for (i in 1:n){
        counts[i] = chi_solve[i, 1] + chi_solve[i, 2] + chi_solve[i, 3] + chi_solve[i, 4] + chi_solve[i, 5] + chi_solve[i, 6];
        donor_counts[i] = chi_solve[i, 5] + chi_solve[i, 6];
        host_counts[i] = chi_solve[i, 1] + chi_solve[i, 2] + chi_solve[i, 3] + chi_solve[i, 4];
        donor_ki[i] = (chi_solve[i, 5])/donor_counts[i];
...
generated quantities{
  ...
  for (i in 1:numPred){
    counts_mean_pred1[i] = y_chi_pred1[i, 1] + y_chi_pred1[i, 2] + y_chi_pred1[i, 3] + y_chi_pred1[i, 4] + y_chi_pred1[i, 5] + y_chi_pred1[i, 6];
    counts_mean_pred2[i] = y_chi_pred2[i, 1] + y_chi_pred2[i, 2] + y_chi_pred2[i, 3] + y_chi_pred2[i, 4] + y_chi_pred2[i, 5] + y_chi_pred2[i, 6];
    counts_mean_pred3[i] = y_chi_pred3[i, 1] + y_chi_pred3[i, 2] + y_chi_pred3[i, 3] + y_chi_pred3[i, 4] + y_chi_pred3[i, 5] + y_chi_pred3[i, 6];
    counts_mean_pred4[i] = y_chi_pred4[i, 1] + y_chi_pred4[i, 2] + y_chi_pred4[i, 3] + y_chi_pred4[i, 4] + y_chi_pred4[i, 5] + y_chi_pred4[i, 6];
    ...
    Nfd_mean_pred1[i] = (y_chi_pred1[i, 5] + y_chi_pred1[i, 6])/(counts_mean_pred1[i] * Chi_spline(ts_pred1[i] - tb_pred1[i]));
    Nfd_mean_pred2[i] = (y_chi_pred2[i, 5] + y_chi_pred2[i, 6])/(counts_mean_pred2[i] * Chi_spline(ts_pred2[i] - tb_pred2[i]));
    Nfd_mean_pred3[i] = (y_chi_pred3[i, 5] + y_chi_pred3[i, 6])/(counts_mean_pred3[i] * Chi_spline(ts_pred3[i] - tb_pred3[i]));
    Nfd_mean_pred4[i] = (y_chi_pred4[i, 5] + y_chi_pred4[i, 6])/(counts_mean_pred4[i] * Chi_spline(ts_pred4[i] - tb_pred4[i]));
    ...
    ki_donor_mean_pred1[i] = (y_chi_pred1[i, 5])/(y_chi_pred1[i, 5] + y_chi_pred1[i, 6]);
    ki_donor_mean_pred2[i] = (y_chi_pred2[i, 5])/(y_chi_pred2[i, 5] + y_chi_pred2[i, 6]);
    ki_donor_mean_pred3[i] = (y_chi_pred3[i, 5])/(y_chi_pred3[i, 5] + y_chi_pred3[i, 6]);
    ki_donor_mean_pred4[i] = (y_chi_pred4[i, 5])/(y_chi_pred4[i, 5] + y_chi_pred4[i, 6]);
