...
real[] shm_chi(real time, real[] y, real[] parms, real[] rdata,  int[] idata) {
  ...
  // Host naive Tregs
  // Thymic tranistionals
  dydt[1] = theta_spline(time, psi) * (1- Chi_spline(time - ageAtBMT)) - (alpha + delta_0) * y[1];
  // Peripheral tranistionals
  dydt[2] = alpha * y[1] - (mu + delta_0) * y[2];
  // Peripheral mature
  dydt[3] = mu * y[2] + alpha * y[4] - (beta + delta) * y[3];
  // Thymic mature
  dydt[4] = beta * y[3] - (alpha + delta) * y[4];

  // Donor naive Tregs
  dydt[5] = theta_spline(time, psi) *  Chi_spline(time - ageAtBMT) - (alpha + delta_0) * y[5];
  // Peripheral tranistionals
  dydt[6] = alpha * y[5] - (mu + delta_0) * y[6];
  // Peripheral mature
  dydt[7] = mu * y[6] + alpha * y[8] - (beta + delta) * y[7];
  // Thymic mature
  dydt[8] = beta * y[7] - (alpha + delta) * y[8];
...
real[] solve_chi(real solve_time, real ageAtBMT, real[] init_cond, real[] parms){
    ...
    init_tb[5] = 0.0;   init_tb[6] = 0.0;    init_tb[7] = 0.0;    init_tb[8] = 0.0;
 ...
 real[,] solve_ode_chi(real[] solve_time, real[] ageAtBMT, real[] init_cond, real[] parms){
  ...
  vector math_reduce(vector global_params, vector local_params, real[] x_r, int[] x_i){
    ...
    //params
    real y1_0 = global_params[7]; real y2_0 = global_params[8];
    real y3_0 = global_params[9]; real y4_0 = global_params[10];
    ...
    real host_counts_thy[n]; real host_counts_per[n];

    vector[4*n] y_mean_stacked;

    // ODE solution -- predictions for the observed timecourse
    init_cond[1] = y1_0; init_cond[2] = y2_0; init_cond[3] = y3_0; init_cond[4] = y4_0;
    init_cond[5] =  0; init_cond[6] = 0; init_cond[7] = 0; init_cond[8] = 0;
    ...
    // PDE solution for chimera dataset -- x_r = data time and x_i = time at BMT
      ...
      for (i in 1:n){
        counts_thy[i] = chi_solve[i, 1] + chi_solve[i, 4] + chi_solve[i, 5] + chi_solve[i, 8];
        counts_per[i] = chi_solve[i, 2] + chi_solve[i, 3] + chi_solve[i, 6] + chi_solve[i, 7];
        donor_counts_thy[i] = chi_solve[i, 5] + chi_solve[i, 8];
        donor_counts_per[i] = chi_solve[i, 6] + chi_solve[i, 7];
...
transformed parameters{
  ...
  global_params[3] = delta_0;
  global_params[4] = mu;
  global_params[5] = beta;
  global_params[6] = delta;
...
generated quantities{
  ...
  for (i in 1:numPred){
    counts_thy_mean_pred1[i] = y_chi_pred1[i, 1] + y_chi_pred1[i, 4] + y_chi_pred1[i, 5] + y_chi_pred1[i, 8];
    counts_thy_mean_pred2[i] = y_chi_pred2[i, 1] + y_chi_pred2[i, 4] + y_chi_pred2[i, 5] + y_chi_pred2[i, 8];
    counts_thy_mean_pred3[i] = y_chi_pred3[i, 1] + y_chi_pred3[i, 4] + y_chi_pred3[i, 5] + y_chi_pred3[i, 8];
    counts_thy_mean_pred4[i] = y_chi_pred4[i, 1] + y_chi_pred4[i, 4] + y_chi_pred4[i, 5] + y_chi_pred4[i, 8];

    counts_per_mean_pred1[i] = y_chi_pred1[i, 2] + y_chi_pred1[i, 3] + y_chi_pred1[i, 6] + y_chi_pred1[i, 7];
    counts_per_mean_pred2[i] = y_chi_pred2[i, 2] + y_chi_pred2[i, 3] + y_chi_pred2[i, 6] + y_chi_pred2[i, 7];
    counts_per_mean_pred3[i] = y_chi_pred3[i, 2] + y_chi_pred3[i, 3] + y_chi_pred3[i, 6] + y_chi_pred3[i, 7];
    counts_per_mean_pred4[i] = y_chi_pred4[i, 2] + y_chi_pred4[i, 3] + y_chi_pred4[i, 6] + y_chi_pred4[i, 7];

    Nfd_thy_mean_pred1[i] = (y_chi_pred1[i, 5] + y_chi_pred1[i, 8])/(counts_thy_mean_pred1[i] * Chi_spline(ts_pred1[i] - tb_pred1[i]));
    Nfd_thy_mean_pred2[i] = (y_chi_pred2[i, 5] + y_chi_pred2[i, 8])/(counts_thy_mean_pred2[i] * Chi_spline(ts_pred2[i] - tb_pred2[i]));
    Nfd_thy_mean_pred3[i] = (y_chi_pred3[i, 5] + y_chi_pred3[i, 8])/(counts_thy_mean_pred3[i] * Chi_spline(ts_pred3[i] - tb_pred3[i]));
    Nfd_thy_mean_pred4[i] = (y_chi_pred4[i, 5] + y_chi_pred4[i, 8])/(counts_thy_mean_pred4[i] * Chi_spline(ts_pred4[i] - tb_pred4[i]));

    Nfd_per_mean_pred1[i] = (y_chi_pred1[i, 6] + y_chi_pred1[i, 7])/(counts_per_mean_pred1[i] * Chi_spline(ts_pred1[i] - tb_pred1[i]));
    Nfd_per_mean_pred2[i] = (y_chi_pred2[i, 6] + y_chi_pred2[i, 7])/(counts_per_mean_pred2[i] * Chi_spline(ts_pred2[i] - tb_pred2[i]));
    Nfd_per_mean_pred3[i] = (y_chi_pred3[i, 6] + y_chi_pred3[i, 7])/(counts_per_mean_pred3[i] * Chi_spline(ts_pred3[i] - tb_pred3[i]));
    Nfd_per_mean_pred4[i] = (y_chi_pred4[i, 6] + y_chi_pred4[i, 7])/(counts_per_mean_pred4[i] * Chi_spline(ts_pred4[i] - tb_pred4[i]));
  }
