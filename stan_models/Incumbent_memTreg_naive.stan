functions{
 //spline 1
 // Timecourse of source population -- naive T cells
 real sp_numbers(real time) {
   real t0 = 40.0;     // mean mouse age at BMT for the first ageBMT bin
   real value;
   // spline fitted separately to the counts of naive cells to estimate the parameters
   real b0  = 7.5; real nu = 0.0042 ;
   //best fitting spline
   value = 10^b0 * exp(-nu * (time-t0));
   return value;
 }

 // Total influx into the memory TReg cell compartment from naive T cells (cells/day)
 real theta_spline(real time, real psi){
  ...
  real Chi_spline(real time) {
    ...
    real value;  real chiEst = 0.9145;   real qEst = 0.021;
...
real donor_eps_spline(real time_post_BMT){
  ...
  real b0 = 0.02; real b1= 0.98; real eps_f = 14;
...
real[] shm_chi(real time, real[] y, real[] parms, real[] rdata,  int[] idata) {
  ...
  real eps_host = 0.018;     // Mean Ki67 hi fraction in host-BM-derived FoxP3 positive Sp4 T cells
