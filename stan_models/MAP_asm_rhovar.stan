...
model{
  ...
  logit(Nfd_naive) ~ normal(logit(to_array_1d(Nfd_naive_mean)), sigma_Nfd);
...
generated quantities{
  ...
  for (i in 1:numPred){
    ...
    Nfd_pred4[i] = logit_inverse(normal_rng(logit(y4_mean_pred4[i]), sigma_Nfd));
  ...
  for (i in 1:numObs2) {
    log_lik_Nfd[i]        = normal_lpdf(logit(Nfd_naive[i]) | logit(Nfd_naive_mean[i]), sigma_Nfd);
  }
  for (i in 1:numObs4) {
