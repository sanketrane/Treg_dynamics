...
parameters{
  real<lower=1E5, upper=2E6> N0;                  // total cells counts at t0
  real<lower=0.0, upper=1.0> delta0;
  real<lower=0.0, upper=delta0> rho;
  real r_del;
...
model{
  N0 ~ normal(5E5, 1.5E5);
  delta0 ~ normal(0.04, 0.2);
  rho ~ normal(0.01, 0.2);
  r_del ~ normal(0.01, 0.05);
