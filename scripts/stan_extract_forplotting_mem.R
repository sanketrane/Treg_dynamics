...
obstimes_index <- sapply(Treg_memory_Nfd$age.at.S1K, find_nearest_time, target_vec = ts_pred1)

Counts_sigma_obs <- Counts_withsigma[obstimes_index, ]%>%
  mutate(ageBMT_bin = Treg_memory_Nfd$ageBMT_bin)
#Counts_pred <- rbind(Counts_pred)
...
Counts_pred <- rbind(Counts_pred)
Counts_withsigma <- rbind(Counts_withsigma)
Nfd_pred <- rbind(Nfd_pred)
