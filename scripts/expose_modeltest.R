...
chi_spline <- function(Time){0.81548689 * (1 - exp(-0.06286984 * (Time - 10)))}
Donor_eps_spline <- function(Time){exp(- 0.06799028 * (Time - 49)) + 0.37848471}


shm_chi <- function (Time, tb, y, parms) {
  eps_host = 0.326611
  kloss = 1/3.5
  with(as.list(c(y, parms)),{
    #dy1 <- Theta_spline(Time, psi) * (1- chi_spline(Time - tb)) * eps_host + rho_D * (2 * y2 + y1) + beta * y3 - (kloss + alpha + delta_D) * y1
    #dy2 <- Theta_spline(Time, psi) * (1- chi_spline(Time - tb)) * (1 - eps_host) + kloss * y1 + beta * y4  - (rho_D + alpha + delta_D) * y2
    ...
    dy9 <- Theta_spline(Time, psi) * chi_spline(Time - tb) * donor_eps_spline(Time)+ rho_D * (2 * y10 + y9) + beta * y11  - (kloss + alpha + delta_D) * y9
    dy10 <- Theta_spline(Time, psi) * chi_spline(Time - tb) * (1 - donor_eps_spline(Time)) + kloss * y9 + beta * y12  - (rho_D + alpha + delta_D) * y10
...
init_cond <- c(y9=0, y10=0, y11=0, y12=0)
ode(y=init_cond, times=c(40, 45), func=shm_chi, parms=params, tb=40)

init_pred1 <- ode(y=init_cond, times=c(40, 45), func=shm_chi, parms=params, tb=40)[2,2:13]
...
R_ode_pred1 <- data.frame(ode(y=init_cond1, times=ts_pred1, func=shm_chi, parms=params, tb=45)) %>%
...
init_pred2 <- ode(y=init_cond, times=c(40, 66), func=shm_chi, parms=params, tb=40)[2,2:13]
...
R_ode_pred2 <- data.frame(ode(y=init_cond2, times=ts_pred2, func=shm_chi, parms=params, tb=66)) %>%
  mutate(time_seq = time,
         counts_thy = y1 + y2 + y7 + y8 + y9 + y10,
         counts_per = y3 + y4 + y5 + y6 + y11 + y12,
         total_counts = counts_thy + counts_per,
...
init_pred3 <- ode(y=init_cond, times=c(40, 76), func=shm_chi, parms=params, tb=40)[2,2:13]
...
R_ode_pred3 <-data.frame(ode(y=init_cond3, times=ts_pred3, func=shm_chi, parms=params, tb=76)) %>%
  mutate(time_seq = time,
         counts_thy = y1 + y2 + y7 + y8 + y9 + y10,
         counts_per = y3 + y4 + y5 + y6 + y11 + y12,
         total_counts = counts_thy + counts_per,
...
init_pred4 <- ode(y=init_cond, times=c(40, 76), func=shm_chi, parms=params, tb=40)[2,2:13]
...
R_ode_pred4 <- data.frame(ode(y=init_cond4, times=ts_pred4, func=shm_chi, parms=params, tb=118)) %>%
  mutate(time_seq = time,
         counts_thy = y1 + y2 + y7 + y8 + y9 + y10,
         counts_per = y3 + y4 + y5 + y6 + y11 + y12,
         total_counts = counts_thy + counts_per,
